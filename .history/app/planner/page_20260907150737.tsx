...
import { destinations, venueTypeLabels, venueTypeIcons, Venue } from '@/lib/data/destinations';
...
export default function EventPlannerPage() {
  ...
  const fetchPlannerData = async () => {
    try {
      ...
      if (preSelectedType && data.eventTypes) {
        const selected = data.eventTypes.find((t: EventType) => t.name === preSelectedType);
        if (selected) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (result.success) {
        router.push(`/thank-you?eventId=${result.eventId}`);
      } else {
          ...
          <div className="max-w-[1280px] mx-auto w-full px-4 sm:px-6 py-10 sm:py-14 lg:py-20 relative z-10 flex flex-col lg:flex-row gap-10 lg:gap-6">
            ...
            <div className="flex-1 flex flex-col gap-10 sm:gap-16 min-w-0">
              ...
              <div className="relative min-h-[400px] bg-charcoal-surface rounded-xl border border-outline-variant/30 p-4 sm:p-6 lg:p-20 overflow-hidden shadow-xl">
                      ...
                      <CustomDropdown
                        ...
                        onChange={(val) => {
                          const allEvents = [
                            ...
                            { id: 'pasni', name: 'Pasni Ceremony' },
                            { id: 'anniversary', name: 'Anniversary' },
                            { id: 'haldi', name: 'Haldi Mehendi' },
                            { id: 'date-planner', name: 'Date Planner' },
                            { id: 'surprise', name: 'Surprise Plan' },
                            { id: 'proposal', name: 'Proposal Setup' },
                            { id: 'corporate', name: 'Corporate Event' },
                            { id: 'inauguration', name: 'Inauguration' },
                            { id: 'others', name: 'Others' },
                          ];
                          const selected = allEvents.find(e => e.id === val);
                        ...
                        options={[
                          ...
                          { value: 'pasni', label: 'Pasni Ceremony', icon: '👶' },
                          { value: 'anniversary', label: 'Anniversary', icon: '💕' },
                          { value: 'haldi', label: 'Haldi Mehendi', icon: '🌼' },
                          { value: 'date-planner', label: 'Date Planner', icon: '🌹' },
                          { value: 'surprise', label: 'Surprise Plan', icon: '🎁' },
                          { value: 'proposal', label: 'Proposal Setup', icon: '💍' },
                          { value: 'corporate', label: 'Corporate Event', icon: '🏢' },
                          { value: 'inauguration', label: 'Inauguration', icon: '🎀' },
                ...
                {currentStep === 3 && (
                  <div className="flex flex-col gap-8 animate-in fade-in">
                    <h2 className="font-display text-cream-contrast text-2xl md:text-3xl font-semibold">Choose Your Theme</h2>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      {themes.map((theme) => (
                        <div
                          key={theme.id}
                          onClick={() => setFormData({ ...formData, themeId: theme.id, themeName: theme.name })}
                          className={`p-6 border-2 rounded-lg hover:border-primary transition cursor-pointer text-center ${formData.themeId === theme.id ? 'border-primary bg-primary/10' : 'border-outline-variant'}`}
                        >
                          <div className="text-3xl mb-3">
                            {theme.name === 'Traditional' && '🏛️'}
                            {theme.name === 'Modern' && '✨'}
                            {theme.name === 'Royal' && '👑'}
                            {theme.name === 'Elegant' && '💎'}
                            {!['Traditional', 'Modern', 'Royal', 'Elegant'].includes(theme.name) && '🎨'}
                          </div>
                          <div className="text-sm font-semibold text-cream-contrast">{theme.name}</div>
                        </div>
                      ))}
                    </div>

                    <div>
                      <label className="text-xs font-bold text-on-surface-variant uppercase tracking-widest mb-4 block">Color Palette</label>
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        {colorPalettes.map((palette) => (
                          <div
                            key={palette.id}
                            onClick={() => setFormData({ ...formData, colorPaletteId: palette.id, colorPaletteName: palette.name })}
                            className={`p-4 border-2 rounded-lg hover:border-primary transition cursor-pointer ${formData.colorPaletteId === palette.id ? 'border-primary bg-primary/10' : 'border-outline-variant'}`}
                          >
                            <div className="flex space-x-1 mb-2">
                              {palette.colors.slice(0, 3).map((color: string, idx: number) => (
                                <div key={idx} className="w-8 h-8 rounded" style={{ backgroundColor: color }} />
                              ))}
                            </div>
                            <div className="text-sm font-semibold text-cream-contrast">{palette.name}</div>
                          </div>
                        ))}
                      </div>
                    </div>
                    ...
                    <div className="mt-auto pt-8 flex flex-col-reverse sm:flex-row gap-3 sm:justify-between">
                      ...
                      <button onClick={nextStep} disabled={!formData.themeId} className="bg-primary text-on-primary text-base font-semibold py-3 px-8 rounded hover:bg-primary-fixed transition-colors flex items-center gap-2 disabled:opacity-50">
                ...
                {currentStep === 5 && (
                  <div className="flex flex-col gap-8 animate-in fade-in">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="bg-surface-container-low p-6 rounded-lg border border-outline-variant">
                        <h3 className="font-semibold text-cream-contrast mb-3">Style Preferences</h3>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                          <div><span className="text-on-surface-variant">Theme:</span> <strong className="text-on-surface">{formData.themeName}</strong></div>
                          <div><span className="text-on-surface-variant">Colors:</span> <strong className="text-on-surface">{formData.colorPaletteName}</strong></div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="mt-auto pt-8 flex flex-col-reverse sm:flex-row gap-3 sm:justify-between">
                      ...
                      <button onClick={handleSubmit} className="bg-primary text-on-primary text-base font-semibold py-3 px-8 rounded hover:bg-primary-fixed transition-colors flex items-center gap-2">
                        Submit Request ✓
                      </button>
                    </div>
                  </div>
