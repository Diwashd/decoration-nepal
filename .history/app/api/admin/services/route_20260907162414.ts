...
export async function PATCH(request: NextRequest) {
  ...
  try {
    const data = await request.json();
    if (!data.id || !data.name || !Number.isFinite(Number(data.basePrice))) {
      return NextResponse.json({ success: false, error: 'Service id, name, and valid price are required' }, { status: 400 });
    }
    ...
    const [service] = await db.update(services).set({
      ...
      updatedAt: new Date(),
    }).where(eq(services.id, data.id)).returning();
