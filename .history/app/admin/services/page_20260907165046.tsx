...
export default function AdminServicesPage() {
  ...
  const [form, setForm] = useState({ name: '', category: 'core', description: '', price: '', icon: '✨', featured: false, image: '' });
  ...
  const openEdit = (service: typeof defaultServices[0]) => {
    ...
    setForm({ name: service.name, category: service.category, description: service.description, price: service.price, icon: service.icon, featured: service.featured, image: (service as typeof service & { image?: string }).image || '' });
    setShowModal(true);
  };

  const openNew = () => {
    setEditingService(null);
    setForm({ name: '', category: 'core', description: '', price: '', icon: '✨', featured: false, image: '' });
    setShowModal(true);
  };
