...
export default function AdminServicesPage() {
  ...
  const [form, setForm] = useState({ name: '', category: 'core', description: '', price: '', icon: '✨', featured: false, image: '', active: true });
  ...
  const openEdit = (service: typeof defaultServices[0]) => {
    ...
    setForm({ name: service.name, category: service.category, description: service.description, price: service.price, icon: service.icon, featured: service.featured, image: (service as typeof service & { image?: string }).image || '', active: service.active });
    setShowModal(true);
  };

  const openNew = () => {
    setEditingService(null);
    setForm({ name: '', category: 'core', description: '', price: '', icon: '✨', featured: false, image: '', active: true });
    setShowModal(true);
  };
        ...
        {filtered.map((service) => (
          <div key={service.id} className={`bg-surface-container border border-outline-variant rounded-xl p-5 transition-all hover:border-primary/30 ${!service.active ? 'opacity-60' : ''}`}>
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => toggleActive(service.id)} className="flex items-center gap-1 px-3 py-1.5 bg-surface-container-high rounded-lg text-sm text-on-surface hover:text-cream-contrast transition-colors">
