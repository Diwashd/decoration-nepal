...
import { useDestinations } from '@/lib/hooks/useAdminStore';
...
export default function AdminDestinationsPage() {
  ...
  const [filter, setFilter] = useState<'all' | 'hotel' | 'restaurant'>('all');
  ...
  const openEdit = (d: typeof defaultDestinations[0]) => {
    setEditing(d);
    setForm({ name: d.name, location: d.location, type: d.type, capacity: d.capacity, priceRange: d.priceRange, description: d.description, contact: d.contact, amenities: d.amenities.join(', '), featured: d.featured, image: d.image || '', order: d.order ?? Number(d.id) });
    setShowModal(true);
  };
  ...
  const saveDestination = () => {
    if (editing) {
      setDestinations(destinations.map(d => d.id === editing.id ? { ...d, ...form, amenities: form.amenities.split(',').map(a => a.trim()) } : d));
    } else {
      setDestinations([...destinations, { ...form, id: String(Date.now()), rating: 4.5, active: true, amenities: form.amenities.split(',').map(a => a.trim()) } as any]);
    }
    setShowModal(false);
  };

  const toggleActive = (id: string) => setDestinations(destinations.map(d => d.id === id ? { ...d, active: !d.active } : d));

  const deleteDestination = (id: string) => { if (confirm('Delete this destination?')) setDestinations(destinations.filter(d => d.id !== id)); };
