...
import { getFeaturedDestinationsFromStore, type AdminStoreDestination } from '@/lib/store/admin-store';
...
export default function FeaturedDestinationsSection() {
  ...
  useEffect(() => {
    const stored = getFeaturedDestinationsFromStore();
    if (stored.length > 0) {
      setFeatured([...stored].sort((a, b) => (a.order ?? Number(a.id)) - (b.order ?? Number(b.id))));
    }
  }, []);
