...
export default function MultiDatePicker({
  ...
  const updateMenuPosition = useCallback(() => {
    if (triggerRef.current) {
      ...
      const width = Math.min(Math.max(rect.width, 280), 360, window.innerWidth - 24);
      const left = Math.min(
    ...
    <div className="flex flex-col relative" ref={dropdownRef}>
      ...
      {isOpen && createPortal(
        <div
          ref={menuRef}
          style={menuStyle}
          className="bg-surface-container-high border border-outline-variant/60 rounded-xl shadow-2xl shadow-black/40 animate-in fade-in duration-150 p-3 sm:p-4"
        >
          <div className="grid grid-cols-1 sm:grid-cols-[minmax(0,1.15fr)_minmax(0,0.85fr)] gap-4">
            <div>
              {/* Calendar Header */}
              <div className="flex items-center justify-between mb-2">
                <button type="button" onClick={prevMonth} className="p-1.5 rounded-lg hover:bg-surface-container-high text-on-surface-variant hover:text-primary transition-colors">
                  <ChevronLeft className="w-3.5 h-3.5" />
                </button>
                <span className="text-xs font-semibold text-cream-contrast">
                  {MONTHS[viewMonth]} {viewYear}
                </span>
                <button type="button" onClick={nextMonth} className="p-1.5 rounded-lg hover:bg-surface-container-high text-on-surface-variant hover:text-primary transition-colors">
            ...
            <div className="border-t sm:border-t-0 sm:border-l border-outline-variant/30 pt-3 sm:pt-0 sm:pl-4 min-w-0">
              <p className="text-[10px] text-on-surface-variant mb-2 font-semibold uppercase tracking-wide">
                {selectedDates.length} date{selectedDates.length !== 1 ? 's' : ''} selected
              </p>
              {selectedDates.length > 0 ? (
                <div className="flex flex-col gap-1 max-h-48 overflow-y-auto pr-1">
                  {[...selectedDates].sort().map(d => (
                    <span key={d} className="flex items-center justify-between gap-2 bg-primary/15 text-primary text-[10px] font-medium px-2 py-1.5 rounded-md">
                      <span className="truncate">{formatDisplayDate(d)}</span>
                      <button type="button" onClick={() => removeDate(d)} className="flex-shrink-0 hover:text-on-surface" aria-label={`Remove ${formatDisplayDate(d)}`}>
                ...
                <p className="text-xs text-on-surface-variant/70">Choose one or more dates from the calendar.</p>
              )}
            </div>
          </div>
        </div>,
